// Static text content for status files.
// Edit here — imported by fileTree.ts.

export const TEXT_PRESENTATION_WELCOME = `\
A OUROBOROS DEFENSE SYSTEMS (ODS) agradece você por comprar e instalar nosso novo terminal v3 de ultima geração de segurança!

Com nosso novo terminal, não só podemos providenciar os nossos Usuários com uma qualidade superior aos nossos competidores nos quesitos de segurança, mas também em conforto de uso. Melhoramos muito desde nosso antigo terminal, reconhecemos que tivemos alguns pequenos problemas com algumas máquinas, mas fiquem feliz em saber que reduzimos o uso de componentes perigosos, com sua saúde e conforto em mente!

Agora usamos:

  - 15% a menos de amianto no gerenciamento de calor do terminal
  - 12% decréscimo no vazamento de radiação gama, em troca a bateria dura 23% menos.
  - A pedidos especiais todos os terminais v3 agora não produzem raios ultravioletas em nenhuma capacidade.
  - Nosso único desenvolvedor agora conseguiu arrumar o bug em que senhas não ficavam salvas após desligar o terminal, ele ganhou até um bis de presente.

─────────────────────────────────────────────

[1] A ODS não recomenda o uso contínuo dos terminais em caso ou suspeita de dengue, gestação, gripe, resfriado, cálculo renal ou doenças similares.

[2] O USUÁRIO, ao utilizar o terminal, declara plena ciência de que a unidade de processamento opera em ambiente de RISCO BIOLÓGICO E RADIOLÓGICO SEVERO, aceitando os seguintes termos:

  §1º - EXCLUSÃO DE RESPONSABILIDADE: A ODS fica integralmente isenta de qualquer responsabilidade civil ou criminal por patologias, mutações, ou falência sistêmica decorrentes da exposição prolongada a este terminal, incluindo, mas não se limitando a: mesotelioma por amianto, necrose por radiação ionizante, ou decomposição molecular espontânea.

  §2º - INEXISTÊNCIA DE GARANTIA DE SAÚDE: O uso desta interface não constitui recomendação médica. Eventuais sangramentos oculares, perda de pigmentação cutânea ou o crescimento de tecidos não identificados são considerados RISCOS INERENTES À OPERAÇÃO e não configuram vício do produto ou defeito de fabricação.

  §3º - RENÚNCIA AO DIREITO DE AÇÃO: O USUÁRIO renuncia, de forma irrevogável e irretratável, ao direito de pleitear indenizações por danos morais, estéticos ou existenciais causados pela inalação de fibras de amianto desprendidas pelo sistema de ventilação ou pelo vazamento de isótopos do núcleo de energia.

  §4º - PRESUNÇÃO DE CULPA EXCLUSIVA: Qualquer sintoma de envenenamento radioativo agudo (Síndrome de Radiação Aguda - SRA) será juridicamente interpretado como CULPA EXCLUSIVA DA VÍTIMA, por falha em manter a distância de segurança de 5 metros.`;

export const TEXT_EMAIL_VAZADO_1 = `\
De: ofertas.sigilosas.saude@promocoesweb.com
Para: kelvinluizcardoso@gmail.com

Cansado de ter um "equipamento" que não impressiona na hora que mais importa? Nossa nova fórmula desenvolvida com nanotecnologia de ponta garante um aumento significativo de comprimento e espessura! Volte a ter o vigor de uma máquina recém-saída da fábrica e energia que dura a noite toda.

✔️ Sem cirurgias caras!
✔️ Sem dor!
✔️ Manutenção zero, potência máxima garantida para o seu motor!

Milhares de homens já fizeram esse "upgrade" e estão vivendo os melhores dias de suas vidas. Não fique para trás!

Ligue agora ou mande uma mensagem rápida para:
(48) 998333697

Fale diretamente com o Joilson! Ele é o nosso consultor VIP e vai preparar o seu kit de forma totalmente discreta e anônima. Estoque limitadíssimo!`;

export const TEXT_EMAIL_VAZADO_2 = `\
De: contato@padariadoisirmaos.com.br
Para: kelvinluizcardoso@gmail.com

Escuta aqui, Kelvin,

Eu já cansei da sua cara de pau. Sou eu, o Ademar, o terceiro irmão que fica no caixa, e eu não aguento mais essa sua mania de "esquecer" de completar o pagamento. Mais uma vez, no pão de queijo de hoje, ficou faltando R$ 1,00.

Você acha que a gente é o quê? Instituição de caridade para chefe do Setor 02? Você vive fazendo isso, Kelvin! É um caloteiro profissional! Um real aqui, cinquenta centavos ali... você está minando o serviço da nossa padaria com essa mesquinharia. Você não tem a menor ideia da dificuldade que é manter esse comércio funcionando, o preço da farinha subindo, a manutenção dos fornos cara, e você aí, com esse cargão na segurança e na mecânica, roubando de quem trabalha duro.

Ou você paga esse real AGORA ou pode esquecer o seu café da manhã de amanhã. Não tem mais fiado, não tem mais "depois eu passo aí". É um desrespeito com a nossa família.

Paga logo essa droga de PIX:
Chave (Telefone): 55 991289793
Favorecido: Padaria Dois Irmãos

Passar bem,
Ademar, o terceiro irmão que fica no caixa.`;

export const TEXT_EMAIL_VAZADO_3 = `\
De: logistica.manutencao.setor02@redeinterna.com
Para: kelvinluizcardoso@gmail.com

Chefe Kelvin,

Aviso urgente de dor de cabeça a caminho. Tivemos um acidente bizarro agora há pouco no Setor 04. Um dos drones de patrulha interna simplesmente perdeu a estabilidade e foi direto na área de descanso. Ele bateu em cheio na máquina de café, quebrando absolutamente tudo. Tem destroços, pó de café e água quente para todo lado.

Ainda não sabemos de quem foi a culpa. Os registros de acesso à bancada daquele drone sumiram ou não foram preenchidos na última revisão, então é impossível apontar o responsável agora. Mas, recolhendo as peças no chão da copa, a equipe percebeu que alguém usou os parafusos errados na fixação dos rotores principais. A vibração soltou o conjunto todo em pleno voo.

O drone já está na baia de manutenção, mas o pessoal do Setor 04 está furioso. Precisamos da sua assinatura no sistema para liberar as peças de reparo do chassi e, principalmente, o formulário de danos ao patrimônio para solicitarmos uma máquina de café nova antes que eles comecem um motim.

Atenciosamente,
Equipe de Manutenção - Turno da Manhã`;

export const TEXT_EMAIL_AUDIO = `\
De: [REMETENTE OCULTO] id_criptografado_883@redeinterna.com
Para: kelvinluizcardoso@gmail.com

Hey Chefia,

Mais um aqui que pode ser interessante. De novo o Claudio e o Filipe. O áudio foi pego na cafeteria agora à tarde.

Dá uma ouvida quando estiver sozinho na sala.

Fico no aguardo de novas instruções, ou se eu ouvir mais algo de interessante, eu te mando, vamos conseguir indenizar essa empresa de merda!
`;

export const TEXT_EMAIL_VAZADO_4 = `\
**De:** juanpablolopes05@gmail.com 
**Para:** kelvinluizcardoso@gmail.com 

Kelvin, meu bem,

Ouvi dizer que o dia hoje está sendo uma verdadeira loucura para você.

Acho simplesmente hilário como o Ademar da padaria perde a linha tão fácil. Ele já te mandou aquele e-mail furioso? O cara vive enchendo a boca para se chamar de "terceiro irmão", mas a verdade é que ele nem da família é! Só porque está enfurnado naquele caixa de padaria há 30 anos, ele acha que é um dos donos. É incrível o que acontece quando "alguém" passa lá de manhã, pega pão de queijo no seu fiado e transfere o PIX deixando faltar exatamente um mísero real de propósito. Ele é muito emocionado, você não acha?

E que tragédia essa do Setor 04, hein? Quem diria que um simples lote de parafusos trocados no almoxarifado ia fazer um drone voar direto na máquina de café deles. Foi um espetáculo. Mas, olhando pelo lado bom... com a equipe de manutenção toda presa na Baia B tentando entender a física por trás do acidente, nós dois ganhamos a desculpa perfeita para fazer "hora extra" hoje.

Eu adoro quando o setor fica um caos e você precisa ficar até mais tarde.

Me encontra no final do turno. Eu vou rastejar pelo acesso atrás do ar-condicionado falso aqui da minha sala e te espero do outro lado da sua estante. Aquele pequeno espaço escuro entre as nossas paredes é o único lugar onde eu quero estar agora.

Estou contando os minutos.

Com todo o meu amor.
**Juan**`;

export const TEXT_EMAIL_BANHEIRO = `\
De: barcellospsicologia@gmail.com
Para: Todos

Prezados colegas,

Preciso comunicar um incidente de manutenção ocorrido esta semana nos banheiros da Unidade.

Foram registrados múltiplos casos de entupimento nos vasos sanitários de três andares distintos. Após inspeção da equipe da Zeladoria, foi identificado como causa principal o acúmulo excessivo de pelos no sistema de tubulação. O volume encontrado, segundo o relatório técnico, foi descrito como "clinicamente perturbador".

Solicito que todos os colaboradores atentem para o descarte adequado de resíduos orgânicos. Os vasos sanitários das instalações não foram projetados para suportar tal volume de material capilar.

A Zeladoria já está trabalhando no desentupimento. O prazo estimado para normalização completa é de 48 horas.

Atenciosamente,
Dr. T.
Departamento de Capital Humano`;

export const TEXT_EMAIL_CLAUDIO_FILIPE = `\
De: claudio.redes@ourobo.ros
Para: filipe.atuadores@ourobo.ros

Filipe,

Preciso te avisar de uma coisa.

Encontrei uma gravação nossa nos logs da rede interna. Não sei quem captou nem como chegou lá, mas está circulando. Anexei o arquivo aqui para você ouvir e me dizer se reconhece o contexto.

Não fala com ninguém sobre isso por enquanto. Dependendo do que você achar, a gente decide o que fazer.

— Cláudio`;

export const TEXT_EMAIL_CAMERA_COPA = `\
De: [REMETENTE OCULTO]
Para: barcellospsicologia@gmail.com

Dr. T.,

Prefiro não me identificar.

Encontrei o que parece ser uma câmera escondida na copa, atrás do painel de ventilação acima da geladeira. O equipamento parece estar em operação.

Fico preocupado com a possibilidade de que conversas privadas realizadas naquele espaço tenham sido gravadas. Você sabe melhor do que ninguém o que é discutido naquele ambiente.

Não sei quem instalou nem para quem as gravações vão. Por isso estou te avisando a você diretamente, e não ao grupo geral do RH.

Cuide-se.`;
